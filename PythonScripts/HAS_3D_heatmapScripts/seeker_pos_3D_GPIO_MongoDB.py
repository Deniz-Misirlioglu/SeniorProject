...
for document in gpioCollection.find():
    seeker_GPIO_x.append(document['xSeeker'])
    seeker_GPIO_y.append(document['ySeeker'])

skipped_seeker_GPIO_x = [x[52:] for x in seeker_GPIO_x]
skipped_seeker_GPIO_y = [y[52:] for y in seeker_GPIO_y]

# combine separated csv into one giant list for x and y
combine_x = [item for sublist in skipped_seeker_GPIO_x for item in sublist]
combine_y = [item for sublist in skipped_seeker_GPIO_y for item in sublist]
